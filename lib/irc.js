...
irc.handle = function(data) {
    ...
    if (new RegExp('PRIVMSG ' + irc.config.nick).test(data)) {
      ...
      if (message.charAt(0) === '!') {
        ...
        if (command === '!help') {
          ...
          } else if (/!nick/.test(command)) {
            var newNick = command.replace(/!nick /, '');
            if (newNick !== '!nick' && newNick !== '!nick ') {
              irc.raw('NICK ' + newNick);
              irc.raw('USER ' + newNick + ' 8 * :' + newNick);
            };
          } else {
  ...
  if (/353/.test(data)) {
    var op = /@\w+/.exec(data)[0].replace(/@/, '');
    if (op !== null) {
      irc.say(irc.config.channel, 'Hey '+op+', can you op me?');
    };
  };
