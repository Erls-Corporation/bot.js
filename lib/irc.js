  ...
  if (/^PING :(.+)$/.test(data)) {
    ...
    if (new RegExp('PRIVMSG ' + irc.config.nick).test(data)) {
      ...
      if (message.charAt(0) === '!') {
        var command = message;
        if (command === '!help') {
          irc.say(irc.config.channel, 'Commands: !topic <topic>, !topic default, !log, !logs <Valid RegExp>, !version');
          ...
          if (/!topic/.test(command)) {
            ...
            irc.say(irc.config.channel, 'This instance is -- ' + irc.version);
          } else {
